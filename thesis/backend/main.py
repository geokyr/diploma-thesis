...
from thesis.backend.routers.drift import drift_router
...
async def lifespan(app: FastAPI):
    try:
        # TODO: might need switching of order and dependency injection
        ...
        drift_client = DriftClient(drift_url=config.drift_url)

        app.state.metrics_store = metrics_store
        app.state.timelapse_driver = timelapse_driver
        app.state.simulation_manager = simulation_manager
        app.state.drift_client = drift_client
        yield
    finally:
        drift_client: DriftClient = getattr(app.state, "drift_client", None)
        ...
        if hasattr(app.state, "drift_client"):
            delattr(app.state, "drift_client")
