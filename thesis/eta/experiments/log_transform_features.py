...
def main() -> None:
    experiment_name = "log_transform_features"
    ...
    for scenario_name, train_path, test_path in SCENARIOS_SPECS:
        ...
        X_train_log, X_test_log = log_transform_features(X_train, X_test)

        models = get_baseline_models()
        results = {}
        for model_name, model in models.items():
            training_results = train_model(model, model_name, X_train_log, y_train)
            predictions, prediction_results = make_predictions(model, model_name, X_test_log)
