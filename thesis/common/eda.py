...
def report_fcd_statistics(df_fcd: pd.DataFrame, id_df: str) -> None:
    """
    ...
    logger.info(f"FCD Statistics - {id_df}")
...
def plot_average_speed_and_traffic_generation_period_per_hour(
    df_fcd_per_hour: pd.DataFrame, id_df: str, plots_dir: Path, traffic_generation_periods: tuple[float, ...]
) -> None:
    """
    ...
    """
    plot_path = plots_dir / f"{id_df}_average_speed_and_traffic_generation_period_per_hour.png"
    ...
    plt.title(f"Average Speed & Traffic Generation Period (Per Hour) - {id_df}")
    plt.savefig(plot_path)
    plt.close()
    logger.info(f"Average speed and traffic generation period per hour saved to {plot_path}")


def report_trips_statistics(df_trips: pd.DataFrame, id_df: str) -> None:
    """
    ...
    logger.info(f"Trips Statistics - {id_df}")
...
def plot_trip_distance_histogram(df_trips: pd.DataFrame, id_df: str, plots_dir: Path, bins: int = 50) -> None:
    """
    ...
    """
    plot_path = plots_dir / f"{id_df}_trip_distance_histogram.png"

    plt.figure(figsize=(6, 4))
    plt.hist(df_trips["distance"], bins=bins, edgecolor="black")
    plt.title(f"Trip Distance Histogram - {id_df}")
...
def plot_trip_duration_histogram(df_trips: pd.DataFrame, id_df: str, plots_dir: Path, bins: int = 50) -> None:
    """
    ...
    Args:
        ...
        bins (int): Number of bins for the histogram.
    """
    plot_path = plots_dir / f"{id_df}_trip_duration_histogram.png"

    plt.figure(figsize=(6, 4))
    plt.hist(df_trips["duration"], bins=bins, edgecolor="black")
    plt.title(f"Trip Duration Histogram - {id_df}")
