...
\begin{enumerate}
    \item \textbf{Original Features (6 features):} Base features extracted directly from the simulation FCD data.
        ...
        \begin{itemize}
            \item \texttt{source\_x}, \texttt{source\_y}, \texttt{destination\_x}, \texttt{destination\_y}, \texttt{time\_start}, \texttt{distance}
        \end{itemize}
    ...
    \item \textbf{Temporal Features (5 features):} Time-based patterns extracted from trip start time.
        ...
        \begin{itemize}
            \item \texttt{hour\_bin}, \texttt{is\_morning}, \texttt{is\_noon}, \texttt{is\_afternoon}, \texttt{is\_rush\_hour}
        \end{itemize}

    \item \textbf{Spatial Features (15 features):} Geometric and geographic relationships derived from coordinates and distances.
        These features capture urban structure and geometric relationships that influence travel time.
        \begin{itemize}
            \item \texttt{x\_center}, \texttt{y\_center}, \texttt{x\_difference}, \texttt{y\_difference}
            \item \texttt{euclidean\_distance}, \texttt{route\_efficiency}, \texttt{detour\_length}
            \item \texttt{trip\_bearing}, \texttt{trip\_bearing\_sin}, \texttt{trip\_bearing\_cos}
            \item \texttt{is\_short\_distance}, \texttt{is\_medium\_distance}, \texttt{is\_long\_distance}
            \item \texttt{source\_distance\_from\_city\_center}, \texttt{destination\_distance\_from\_city\_center}
            \item \texttt{trip\_centrality\_change}, \texttt{trip\_centrality}
        \end{itemize}
    ...
    \item \textbf{Fourier Features (16 features):} Sinusoidal positional encoding of spatial coordinates at 2 frequency scales.
        ...
        Listed only the 8 features for the source coordinates, since the destination coordinates ones are symmetric.
        \begin{itemize}
            \item \texttt{source\_x\_sin\_0}, \texttt{source\_x\_cos\_0}, \texttt{source\_x\_sin\_1}, \texttt{source\_x\_cos\_1}
            \item \texttt{source\_y\_sin\_0}, \texttt{source\_y\_cos\_0}, \texttt{source\_y\_sin\_1}, \texttt{source\_y\_cos\_1}
        \end{itemize}
    ...
    \item \textbf{Cell Features (4 features):} Spatial discretization into fixed-size grid cells (100 meters).
        ...
        \begin{itemize}
            \item \texttt{source\_cell\_x}, \texttt{source\_cell\_y}, \texttt{destination\_cell\_x}, \texttt{destination\_cell\_y}
        \end{itemize}
    ...
    \item \textbf{Cluster Features (2 features):} K-Means clustering (K=20) on coordinates to discretize spatial regions.
        ...
        \begin{itemize}
            \item \texttt{source\_cluster}, \texttt{destination\_cluster}
        \end{itemize}
    ...
    \item \textbf{PCA Features (4 features):} Principal Component Analysis for dimensionality reduction (2 components).
        ...
        \begin{itemize}
            \item \texttt{source\_pca\_1}, \texttt{source\_pca\_2}, \texttt{destination\_pca\_1}, \texttt{destination\_pca\_2}
        \end{itemize}
\end{enumerate}
...
Based on the combined ranking and the correlation analysis, we decided to eliminated all temporal features, all fourier features, all cell features 7 of the spatial features.

The retained features are the 6 original features, the 2 cluster features, the 4 pca features and the following 10 spatial features:
\begin{itemize}
    \item \texttt{x\_center}
    \item \texttt{y\_center}
    \item \texttt{euclidean\_distance}
    \item \texttt{detour\_length}
    \item \texttt{is\_short\_distance}
    \item \texttt{is\_medium\_distance}
    \item \texttt{is\_long\_distance}
    \item \texttt{source\_distance\_from\_city\_center}
    \item \texttt{destination\_distance\_from\_city\_center}
    \item \texttt{trip\_centrality}
\end{itemize}
