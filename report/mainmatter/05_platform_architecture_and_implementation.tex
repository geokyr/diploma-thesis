...
\begin{itemize}
    \item \textbf{Backend} (Port 8000): orchestration, timelapse control, state stores, simulation snapshot API.
    \item \textbf{Predictor-ETA/Fuel/Stops} (Port 8001/8002/8003): inference, feature calibration, batch or single predictions, background retraining.
    \item \textbf{Drift} (Port 8004): per-task workers, four detectors, consensus and online calibration.
    \item \textbf{Summarizer} (Port 8005): post-simulation AI report generation from metrics and notifications.
    \item \textbf{Frontend} (Port 8080): admin dashboard and user interface, interacts only with the Backend API.
\end{itemize}

\autoref{fig:platform-architecture} showcases the architecture as a diagram:
...
\begin{itemize}
    \item \textbf{Infrastructure:} Docker, Docker Compose, uv dependency manager~\cite{astral2025uv}, Python 3.12
...
\autoref{fig:drift-states} shows the various drift states of a model, as the simulation progresses, in a diagram:

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{figures/drift-states.png}
    \caption{Drift States of a Model.}
    \label{fig:drift-states}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/swagger-backend.png}
    \caption{Backend API Endpoints Reference, Swagger UI.}
    \label{fig:swagger-backend}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/swagger-predictor.png}
    \caption{Predictor API Endpoints Reference, Swagger UI.}
    \label{fig:swagger-predictor}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/swagger-drift.png}
    \caption{Drift API Endpoints Reference, Swagger UI.}
    \label{fig:swagger-drift}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/swagger-summarizer.png}
    \caption{Summarizer API Endpoints Reference, Swagger UI.}
    \label{fig:swagger-summarizer}
\end{figure}

\paragraph{Frontend Service}
The frontend provides a web interface with Admin Dashboard and User Interface tabs. It communicates with the backend exclusively through the backend's REST API endpoints. It does not expose any REST API endpoints itself. The interface is accessible at \texttt{http://localhost:8080}.
