...
\subsection{Microservices Topology}
...
\begin{itemize}
    \item Backend (8000): orchestration, timelapse control, state stores, simulation snapshot API.
    \item Predictor-ETA (8001), Predictor-Fuel (8002), Predictor-Stops (8003): inference, feature calibration, batch/single predictions, background retraining.
    \item Drift (8004): per-task workers, four detectors, consensus and online calibration.
    \item Summarizer (8005): post-simulation AI report generation from metrics and notifications.
    \item Frontend (8080): admin dashboard and user interface, interacts only with the Backend API.
\end{itemize}
...
Compose ensures startup ordering via health checks, isolates the network, and mounts shared volumes under \texttt{appdata/} (common SUMO data, task datasets, model registry, feature artifacts, and per-service logs).
