...
from thesis.eta.features import log_transform_features, split_features_and_target
...
def main() -> None:
    ...
    for model_type in ModelType:
        ...
        for train_index, val_index in skf.split(X_train, stratify_key):
            ...
            X_train_fold_log, X_val_fold_log = log_transform_features(X_train_fold, X_val_fold)

            training_results = train_model(model, model_type, X_train_fold_log, y_train_fold)
            predictions, prediction_results = make_predictions(model, model_type, X_val_fold_log)
        ...
        X_train_log, _ = log_transform_features(X_train)
        train_model(final_model, model_type, X_train_log, y_train)
