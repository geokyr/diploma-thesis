...
def main() -> None:
    ...
    for model_type in ModelType:
        ...
        for train_index, val_index in skf.split(X_train, stratify_key):
            ...
            X_train_fold_scaled, X_val_fold_scaled = standard_scale_features(X_train_fold, X_val_fold)

            training_results = train_model(model, model_type, X_train_fold_scaled, y_train_fold)
            predictions, prediction_results = make_predictions(model, model_type, X_val_fold_scaled)
        ...
        X_train_scaled, _ = standard_scale_features(X_train)
        train_model(final_model, model_type, X_train_scaled, y_train)
